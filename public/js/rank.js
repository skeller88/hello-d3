 ...
 * @param chartOptions
 */
function drawRankChart(data, baselineName, comparisonName) {
    var isBetweenGroupComparison = !!data.comparisonData;
    var title = 'Number of ' + 'symptoms';
    var xLabel = 'Number of ' + 'symptoms';
    var yLabel = 'Count Ranking';

    if (isBetweenGroupComparison) {
        var baselineCount = d3.sum(data.baselineData.values, function(d) {
            return parseInt(d.count);
        });

        var comparisonCount = d3.sum(data.comparisonData.values, function(d) {
            return parseInt(d.count);
        });
        ...
        var baselineCount = data.n_users;
